...
def get_img_dataloader(args):
    ...
    for i in range(len(names)):
        if i in args.test_envs:
            ...
            datasets = ImageDataset(args.dataset, args.task, args.data_dir,
                                    names[i], i, transform=image_train(args), indices=indextr,test_envs=args.test_envs,shots_per_class=args.shots_per_class)
            trdatalist.append(datasets)
            tedatalist.append(ImageDataset(args.dataset, args.task, args.data_dir,
                                           names[i], i, transform=image_test(args), indices=indexte, test_envs=args.test_envs))  # val
    ...
    eval_loaders = [DataLoader(
        ...
        drop_last=False,
        shuffle=False)
        for env in trdatalist + tedatalist]  # eval_loaders=[loader1, loader2] all_loaders
